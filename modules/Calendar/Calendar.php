...
class Calendar
{
    ...
    public $views = array("agendaDay" => array(), "basicDay" => array(), "basicWeek" => array(), "agendaWeek" => array(), "month" => array(), "sharedMonth" => array(), "sharedWeek" => array(), "sharedWeekAll" => array(), "sharedMonthAll" => array());
    ...
    public $filter_field='All';

    public $filter_users='All';
    ...
    public function load_activities()
    {
        ...
        if ($this->view == 'sharedWeekAll' || $this->view == 'sharedMonthAll') {
            $this->filter_field = $current_user->getPreference('filter_users_occupation');
            $this->filter_users = $current_user->getPreference('filter_users');
        }

        $i = 0;
        foreach ($this->acts_arr as $user_id => $acts) {

            if ($this->filter_users != $calendarStrings['LBL_ALL_INFORMATION'])
                if ($user_id != $this->filter_users) continue;
            ...
            foreach ($acts as $act) {
                if ($this->filter_field != $calendarStrings['LBL_ALL_INFORMATION'])
                    if ($act->sugar_bean->module_dir != $this->filter_field) {
                        continue;
                    }
                $item = array();
    ...
    public function init_shared()
    {
        global $current_user;
        $calendarStrings = return_module_language($GLOBALS['current_language'], 'Calendar');
        if (isset($_REQUEST['filter_users']) && !empty($_REQUEST['filter_users']))
            $current_user->setPreference('filter_users', $_REQUEST['filter_users']);
        else $this->filter_users= $calendarStrings['LBL_ALL_INFORMATION'];
        if (!empty($_REQUEST['filter_users_occupation']) && isset($_REQUEST['filter_users_occupation']))
            $current_user->setPreference('filter_users_occupation', $_REQUEST['filter_users_occupation']);
        else $this->filter_field= $calendarStrings['LBL_ALL_INFORMATION'];
        ...
        if ($this->view == 'sharedWeekAll' || $this->view == 'sharedMonthAll') {
            ...
            $array_of_users = get_user_array(false);
            $array_of_users = array_keys($array_of_users);
            $this->shared_ids = $array_of_users;
        }
    }
    ...
    public function add_activities($user, $type = 'sugar')
    {
        ...
        if ($this->view == 'agendaWeek' || $this->view == 'sharedWeek' || $this->view == 'sharedWeekAll') {
            $start_date_time = CalendarUtils::get_first_day_of_week($this->date_time);
            $end_date_time = $start_date_time->get("+7 days");
        } else if ($this->view == 'month' || $this->view == "sharedMonth" || $this->view == "sharedMonthAll") {
    ...
    public function get_neighbor_date_str($direction)
    {
        ...
        if ($this->view == 'month' || $this->view == "sharedMonth" || $this->view == "sharedMonthAll") {
            $day = $this->date_time->get_day_by_index_this_month(0)->get($sign . "1 month")->get_day_begin(1);
        } else if ($this->view == 'agendaWeek' || $this->view == 'sharedWeek' || $this->view == 'basicWeek' || $this->view == 'sharedWeekAll') {
